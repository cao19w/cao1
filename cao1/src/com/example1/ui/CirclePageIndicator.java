...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCentered) {
            longOffset += ((longSize - longPaddingBefore - longPaddingAfter) / 2.0f) - ((count * threeRadius) / 2.0f)-((count*mExtraSpace)/2.0f);
        }
        ...
        for (int iLoop = 0; iLoop < count; iLoop++) {
            float drawLong = longOffset + (iLoop * (threeRadius+mExtraSpace));
            if (mOrientation == HORIZONTAL) {
                dX = drawLong;
                dY = shortOffset;
            } else {
        ...
        float cx = (mSnap ? mSnapPage : mCurrentPage) * (threeRadius+mExtraSpace);
        if (!mSnap) {
            cx += mPageOffset * (threeRadius+mExtraSpace);
        }
        if (mOrientation == HORIZONTAL) {
            dX = longOffset + cx;
            dY = shortOffset;
    ...
    private int measureLong(int measureSpec) {
        ...
        if ((specMode == MeasureSpec.EXACTLY) || (mViewPager == null)) {
            ...
            result = (int)(getPaddingLeft() + getPaddingRight()
                    + (count * 2 * mRadius) + (count - 1) * mRadius+(count-1)*mExtraSpace + 1);
